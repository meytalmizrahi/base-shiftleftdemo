...
stage("Scan Cloud Formation Template with API v2") {
	...
	stage('Checkov') {
		try {
			...
			response = sh(script:"checkov --file files/deploy.yml --bc-api-key $BC_API --repo-id <meytalmizrahi/base-evil.petclinic> -b main -o junitxml > result.xml || true", returnStdout:true).trim() // -o junitxml > result.xml || true"
            			}
